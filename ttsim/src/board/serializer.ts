...
import {transformToMatrix} from "../custom/matrixTransformer";
...
export class URLBoardSerializer implements IBoardSerializer {
    ...
    protected onChange(): void {
        ...
        this._trySaveInterval = setInterval(
            this.save.bind(this),
    ...
    public async download() {
        ...
        const matrix = await transformToMatrix(url);

        try {
            const res = await fetch(
                "http://127.0.0.1:8000/TranslationLayer/t2/" + JSON.stringify(matrix),
                {
                    method: "GET",
                }
            );
        ...
        console.log(await transformToMatrix(url));
    ...
    protected _writeBoardState(): string {
        ...
        const metadataRows: number = Math.ceil(
            metadata.length / metadataBytesPerRow
        );
        ...
        for (let r: number = this.board.rowCount; r < canvas.height; r++) {
            ...
            metadata = this.writeMetadataRow(
                imageData.data,
                r * rw,
                canvas.width,
                metadata
    ...
    protected colorToPart(data: Uint8ClampedArray, i: number): Part {
        ...
        if (max == 0) type = PartType.INTERCEPTOR;
        // black
        else {
            r /= max;
            g /= max;
            ...
            if (isHigh(r) && isHigh(g) && isHigh(b)) {
                ...
                // colors
                if (isLow(r) && isHigh(g) && isLow(b)) type = PartType.RAMP;
                // green
                else if (isHigh(r) && isMid(g) && isLow(b)) type = PartType.CROSSOVER;
                // orange
                else if (isLow(r) && isHigh(g) && isHigh(b)) type = PartType.BIT;
                // cyan
                else if (isMid(r) && isLow(g) && isHigh(b)) type = PartType.GEARBIT;
                // purple
                else if (isHigh(r) && isLow(g) && isLow(b)) type = PartType.GEAR;
                // red
                else if (isHigh(r) && isHigh(g) && isLow(b)) type = PartType.DROP;
                // yellow
