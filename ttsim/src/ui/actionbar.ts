...
export class Actionbar extends ButtonBar {
  constructor(public readonly board: Board) {
    ...
    this._matrixSendButton = new SpriteButton(
      new PIXI.Sprite()
  ...
  private _matrixSendButton: Button;
  ...
  protected async onButtonClick(button: Button) {
    if (button === this._schematicButton) {
      ...
      const matrix = await transformToMatrix(board);
      const body = {
        array: matrix,
      };
      const strBody = JSON.stringify(body);
      const resp = await fetch(
        "http://127.0.0.1:8000/TranslationLayer/t2/" + strBody,
      );
      console.log(resp);
    }
  }
